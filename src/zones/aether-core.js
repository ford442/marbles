...
export function createAetherCoreZone(game, offset) {
    const zone = game;
    const pos = offset;
    const F = zone.game?.Filament || zone.Filament;

    console.log('[ZONE] Creating Aether Core zone at', pos);

    // 1. Entrance Platform
    zone.createFloorZone({ x: pos.x, y: pos.y, z: pos.z - 30 }, {
        width: 10, depth: 10, friction: 0.8, color: [0.1, 0.1, 0.2]
    });
    ...
    // 3. Aether Cores (Kinematic Moving Platforms)
    for (let i = 0; i < 3; i++) {
        ...
        const color = [0.1, 0.8, 0.9]; // Cyan glowing platforms
    ...
    // 4. Aether Updraft (Jump Pad)
    zone.createJumpZone({ x: pos.x, y: pos.y - 12, z: pos.z + 80 }, {
        width: 10, depth: 10,
        boostForce: 50,
        color: [0.9, 0.1, 0.9] // Neon purple
    });
    ...
    console.log('[ZONE] Aether Core zone created successfully');
}
