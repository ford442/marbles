...
export function createMeteoriteHollowZone(game, offset) {
    ...
    // --- Frozen Lake (Low Friction Icy Floor) ---
    {
        ...
        const { entity } = createVisual(pos, hExtents, [0.3, 0.8, 1.0], 0.0); // Icy blue glass
    ...
    // --- Emissive Central Meteorite Light ---
    ...
    // --- Orbiting/Moving Meteorite Blocks (Kinematic) ---
    for (let i = 0; i < 6; i++) {
        const hExtents = { x: 2, y: 2, z: 2 };
        const zPos = offset.z + 10 + i * 10;
        const yPos = offset.y - 1;
        ...
        const colliderDesc = RAPIER.ColliderDesc.cuboid(hExtents.x, hExtents.y, hExtents.z).setRestitution(0.5);
        game.world.createCollider(colliderDesc, body);
        audio.registerBodyMaterial(body, 'rock');

        // Bumpy glowing hot rock
        const { entity } = createVisual(pos, hExtents, [0.8, 0.3, 0.1], 0.8, true);
        ...
        game.movingPlatforms.push({
            ...
            amplitude: 8.0,
            initialPos: { ...pos },
            speed: direction * (1.5 + Math.random() * 0.5)
        });
    }
