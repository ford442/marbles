...
export function createCrystalCavernZone(game, offset) {
    ...
    // --- Entrance Platform ---
    {
        ...
        const { entity } = createVisual({ x: offset.x, y: offset.y, z: offset.z }, hExtents, [0.15, 0.15, 0.2]);
    ...
    // --- Low Friction Ice Track Segment (Crystal Blue) ---
    {
        ...
        // Crystal blue visual
        const { entity } = createVisual(pos, hExtents, [0.1, 0.7, 1.0], 0.0);
    ...
    // --- Kinematic Moving Platforms (Glowing Purple Crystals) ---
    const platformCount = 4;
    for (let i = 0; i < platformCount; i++) {
        const hExtents = { x: 4, y: 0.5, z: 4 };
        const zOffset = offset.z + 45 + (i * 15);
        ...
        const { entity } = createVisual({ x: startX, y: offset.y - 2, z: zOffset }, hExtents, [0.5, 0.1, 1.0], 0.1);
