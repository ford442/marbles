...
export function createVoidStationZone(game, offset) {
    ...
    // --- Zero-G Glass Track ---
    {
        const hExtents = { x: 4, y: 0.5, z: 20 };
        const pos = { x: offset.x, y: offset.y - 2, z: offset.z + 25 };
        const bodyDesc = RAPIER.RigidBodyDesc.fixed()
        ...
        const { entity } = createVisual(pos, hExtents, [0.8, 0.1, 0.9], 0.0);
    ...
    // --- Kinematic Moving Obstacles (Void Rings) ---
    for (let i = 0; i < 3; i++) {
        const hExtents = { x: 3, y: 0.5, z: 0.5 };
        const zOffset = offset.z + 15 + (i * 10);
        ...
        const { entity } = createVisual({ x: startX, y: offset.y - 1.5, z: zOffset }, hExtents, [0.1, 1.0, 0.5], 0.2);
